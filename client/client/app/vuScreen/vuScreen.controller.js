    ...
    if ($('#dau-data-data-table').length !== 0) {
        ...
        var dautable = $('#dau-data-data-table').DataTable({
            ...
            columns: [
                ...
                { data: "battery_level", searchBy: true },
                { data: "number_of_unique_user", searchBy: true },
                { data: "home_page_user", searchBy: true },
                { data: "journey_id" },
                { data: "sync_latitude", visible: false },
                { data: "sync_longitude", visible: false },
                { data: "tail_number", searchBy: true, visible: false },
                { data: "location_selection",searchBy: true, visible: false },
    ...
    if ($('#dau-data-data-table').length !== 0) {
        ...
        var dautable = $('#dau-data-data-table').DataTable({
            ...
            columns: [
                ...
                { data: "latitude" }, 
                { data: "longitude" },
                { data: "mac_address", searchBy: true },
                {data:"journey_id"},
...
colorAdminApp.controller('openformController', function ($scope, $interval, $rootScope, $http, $cookieStore) {
    function pad(s) { return (s < 10) ? '0' + s : s; }
    var d = new Date();
    var currentDate = new Date().toISOString().slice(0, 10)
    $scope.startDate = currentDate;
    ...
    if ($('#dau-data-data-table').length !== 0) {
        ...
        var dautable = $('#dau-data-data-table').DataTable({
            ...
            columns: [
                ...
                { data: "pincode", visible: false },
                { data: "performance", visible: false },
                { data: "tutions",visible: false },
                { data: "studies", visible: false},
                { data: "mac_address", searchBy: true },
                {data:"journey_id"},
...
colorAdminApp.controller('smshostreportController', function ($scope, $interval, $rootScope, $http, $cookieStore) {
    function pad(s) { return (s < 10) ? '0' + s : s; }
    var d = new Date();
    var currentDate = new Date().toISOString().slice(0, 10)
    $scope.startDate = currentDate;
    ...
    if ($('#dau-data-data-table').length !== 0) {
        ...
        var dautable = $('#dau-data-data-table').DataTable({
            ...
            buttons: [
                // { extend: 'copy', className: 'btn-sm'},
                { extend: 'csv', className: 'btn-sm', exportURL: '/api/vuscreen/Knowlarity_Passcode/export/csv' },
                // { extend: 'pdf', className: 'btn-sm'},
                { extend: 'colvis', className: 'btn-sm' },
            ],
            processing: true,
            serverSide: true,
            ajax: {
                url: "/api/vuscreen/Knowlarity_Passcode",
                data: function (d) {
                    d.startDate = $scope.startDate;
                    d.endDate = $scope.endDate;
                    d.access_token = $cookieStore.get('token');
...
colorAdminApp.controller('smsleadreportController', function ($scope, $interval, $rootScope, $http, $cookieStore) {
    function pad(s) { return (s < 10) ? '0' + s : s; }
    var d = new Date();
    var currentDate = new Date().toISOString().slice(0, 10)
    $scope.startDate = currentDate;
    ...
    if ($('#dau-data-data-table').length !== 0) {
        ...
        var dautable = $('#dau-data-data-table').DataTable({
            ...
            buttons: [
                // { extend: 'copy', className: 'btn-sm'},
                { extend: 'csv', className: 'btn-sm', exportURL: '/api/vuscreen/Knowlarity_ana/export/csv' },
                // { extend: 'pdf', className: 'btn-sm'},
                { extend: 'colvis', className: 'btn-sm' },
            ],
            processing: true,
            serverSide: true,
            ajax: {
                url: "/api/vuscreen/Knowlarity_ana",
                data: function (d) {
                    d.startDate = $scope.startDate;
                    d.endDate = $scope.endDate;
                    d.access_token = $cookieStore.get('token');
            ...
            columns: [
                ...
                { data: "pdfUser" },
                { data: "pdfClick" },
                { data: "travelUser", visible: false },
                { data: "travelClick", visible: false },
                { data: "gameUser", visible: false },
                { data: "gameClick", visible: false },
                { data: "fnbUser", visible: false },
                { data: "fnbClick", visible: false },
                { data: "version", visible: false},
                { data: "jsonUpdateDate",visible: false },
                { data: "jsonUpdateTime", visible: false }
...
colorAdminApp.controller('serverSessionController', function ($scope, $rootScope, $state, Auth, $location, $cookieStore, blackbox, $http) {
    ...
    $scope.downloadFile = function (data, filename = 'serverSession') {
        let csvData = ConvertToCSV(data, ['view_date', 'HostID', 'source', 'destination', 'cycle', 'wifiLogin', 'start_date', 'start_time', 
